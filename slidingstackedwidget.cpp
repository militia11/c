...
void SlidingStackedWidget::slideInIdx(int idx, enum t_direction direction) {
        int trueIdx = 0;
        if(mappedViews.contains(idx))
            trueIdx = mappedViews[idx];
        else
            trueIdx = idx;
        //int idx, t_direction direction=AUTOMATIC
        if (trueIdx>count()-1) {
            direction=m_vertical ? TOP2BOTTOM : RIGHT2LEFT;
            trueIdx=(trueIdx)%count();
        }
        else if (trueIdx<0) {
            direction= m_vertical ? BOTTOM2TOP: LEFT2RIGHT;
            trueIdx=(trueIdx+count())%count();
        }
        slideInWgt(widget ( trueIdx ),direction);
...
int SlidingStackedWidget::addWidget(int mapViewIdx, QWidget* widget)
{
    const int resultIdx = QStackedWidget::addWidget(widget);
    mappedViews[mapViewIdx] = resultIdx;
    return resultIdx;
}
...
void SlidingStackedWidget::removeWidget(QWidget* widget)
{
    ...
    const int mappedIdx = mappedViews.key(idx, -1);
    if(mappedIdx != -1)
        mappedViews.remove(mappedIdx);
}
